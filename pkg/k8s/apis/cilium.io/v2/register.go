...
func CreateCustomResourceDefinitions(clientset apiextensionsclient.Interface) error {
	...
	// If the CRD already exists on the cluster but without any validation then
	...
	if errors.IsAlreadyExists(err) && clusterCRD.Spec.Validation == nil {
		...
		err = wait.Poll(500*time.Millisecond, 60*time.Second, func() (bool, error) {
			...
			if err != nil {
				return false, err
			} else if !reflect.DeepEqual(clusterCRD.Spec.Validation.OpenAPIV3Schema, crv.OpenAPIV3Schema) {
				clusterCRD.Spec.Validation = res.Spec.Validation
				log.Infof("CRD validation is different, updating it...")
		...
		if err != nil {
			log.Errorf("Unable to update v2.CiliumNetworkPolicy %s", err)
		}
	}
	...
	log.Info("Creating v2.CiliumNetworkPolicy CustomResourceDefinition... DONE")
...
var (
	...
	properties = map[string]apiextensionsv1beta1.JSONSchemaProps{
		...
		"CiliumNetworkPolicyNodeStatus": {
			Description: "CiliumNetworkPolicyNodeStatus is the status of a Cilium policy rule for " +
				"a specific node",
			Properties: map[string]apiextensionsv1beta1.JSONSchemaProps{
				"error": {
					Description: "Error describes the error condition if OK is false",
					Type:        "string",
				},
				"lastUpdated": {
					Description: "LastUpdated contains the last time this status was updated",
					Ref:         getStr("#/properties/Timestamp"),
				},
				"ok": {
					Description: "OK is true when the policy has been installed successfully",
					Type:        "boolean",
				},
			},
		},
		"CiliumNetworkPolicyStatus": {
			Description: "CiliumNetworkPolicyStatus is the status of a Cilium policy rule",
			Properties: map[string]apiextensionsv1beta1.JSONSchemaProps{
				"nodes": {
					Description: "Nodes is the Cilium policy status for each node",
					Type:        "object",
					Ref:         getStr("#/properties/CiliumNetworkPolicyNodeStatus"),
				},
			},
		},
		"EgressRule": {
		...
		"PortRuleKafka": {
			...
			Properties: map[string]apiextensionsv1beta1.JSONSchemaProps{
				...
				"apiKeyInt": {
					Description: "apiKeyInt is the integer representation of APIKey",
					Type:        "integer",
					Format:      "int32",
				},
				"apiVersion": {
				...
				"apiVersionInt": {
					Description: "apiVersionInt is the integer representation of APIVersion",
					Type:        "integer",
					Format:      "int32",
				},
				"clientID": {
		...
		"status": {
			...
			Ref:         getStr("#/properties/CiliumNetworkPolicyStatus"),
		},
		"Timestamp": {
			Description: "Timestamp is a wrapper of time.Time so that we can create our own " +
